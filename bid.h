...
void singlegaming(player p[],int bankcounter, int N, card* general ) // ���a���A �B���a���֡B���a�ơB���P
{
...
void initial(player p[], const int bank, int &moneypot, bool fold[],int N)
{
    ...
    p[bank].plus_bitThisRound(5); //�j���`
    p[bank].plus_chips(-5);
    p[(bank+1) % N].plus_bitThisRound(3); //�p���`
    p[(bank+1)%N].plus_chips(-3);

    moneypot += 8;
    int highest = 5;
    int pass = 0;
    int folder = 0; // fold�ΨӰO���� folder �ΨӰO������
